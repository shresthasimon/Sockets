...
class MyTCPHandler(socketserver.BaseRequestHandler):	
	...
	def handle(self):
		...
		if message[0] == 'msg':
			...
			source = message[3]
			hops = int(message[4])
			package = ''.join(message[5:])

			if target == NID:
				os.system('clear')
				print(origin + ' says: ' + package)
				os.system("""bash -c 'read -s -n 1 -p "Press any key to continue..."'""")

			else:
				sendto(target, origin, str(NID), hops, package)
			...
			for item in temp_routes:
				...
				if item not in routes and item != NID:
					...
					if temp_routes[item][0] != -1:
						...
						# add null route
						routes[item] = (temp_routes[item][0], temp_routes[item][1])
				...
				if item in routes:
					...
					if temp_routes[item][1] == -1:
						...
						# update with null route
						routes[item] = (temp_routes[item][0], temp_routes[item][1])

			# for item in routes:
			# 	if item in temp_routes and temp_routes[item][0] == -1:
			# 		routes[item][0] == -1
			# 		routes[item][1] == 10
			# 		remove_counter += 1
...
# Function: sendto()
def sendto(dest_nid, src_nid, source, hops, message):
	...
	package = pickle.dumps('msg' + ' ' + str(dest_nid) + ' ' + src_nid + ' ' + str(NID) + ' ' + str(hops) + ' ' + message)
	...
	if hops > 0:
		...
		try:
			print('received from: ' + source + ', forwarding to ' + str(gateway))
...
def main(argv):
	...
	while(run):
		...
		#print menu options
		os.system('clear')
		print("Enter '1' to check node status")
		print("Enter '2' to message another node")
		...
		# selection: send
			...
			sendto(int(dest_node), str(NID), str(NID), 15, message)
			os.system("""bash -c 'read -s -n 1 -p "Press any key to continue..."'""")
